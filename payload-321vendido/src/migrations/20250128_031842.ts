...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`rematadores\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`address\` text,
  	\`phone\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`rematadores_logo_idx\` ON \`rematadores\` (\`logo_id\`);`)
  await db.run(sql`CREATE INDEX \`rematadores_updated_at_idx\` ON \`rematadores\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`rematadores_created_at_idx\` ON \`rematadores\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`remates\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`rematador_id\` integer NOT NULL,
  	\`description\` text,
  	...
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`rematador_id\`) REFERENCES \`rematadores\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`cover_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`remates_rematador_idx\` ON \`remates\` (\`rematador_id\`);`)
  await db.run(sql`CREATE INDEX \`remates_cover_image_idx\` ON \`remates\` (\`cover_image_id\`);`)
  await db.run(sql`CREATE INDEX \`remates_updated_at_idx\` ON \`remates\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`remates_created_at_idx\` ON \`remates\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`categories_id\`) REFERENCES \`categories\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`rematadores_id\`) REFERENCES \`rematadores\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`remates_id\`) REFERENCES \`remates\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_rematadores_id_idx\` ON \`payload_locked_documents_rels\` (\`rematadores_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_remates_id_idx\` ON \`payload_locked_documents_rels\` (\`remates_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`rematadores\`;`)
  await db.run(sql`DROP TABLE \`remates\`;`)
